...
- **`core/`** (`harvest_core`) — never deployed alone. Contains, in dependency order:
  ...
  - `ports.py` — one `typing.Protocol` per external system (Datastore, TaskQueue, KeyValue, ObjectStore, PubSub, SearchProvider, QueryGenerator, Triage, Fetcher, PortalDiscoverer, CensusSource, Clock). `PubSub` is deliberately separate from `KeyValue`: a subscribed Redis connection cannot run ordinary commands, so a `Subscription` owns one and `close()` is what returns it.
  - `health.py` — the container health check both deployables answer, since neither has an HTTP listener to probe. The probe is a second process in the same container (`<service> healthcheck`) that pings over Redis pub/sub; the service replies with a status built from its own worker threads and loop `Beat`, so a dead consumer fails the check while the process is still up. Both `healthcheck` subcommands deliberately skip their service's wiring — that would run the boot migration every thirty seconds.
...
- **The orchestrator is the only component that originates work** (`seed_pass`, `poll_mail`, `followup_scan`, `send_digests`), each concern single-flight per tick — a run still in flight is skipped, not queued. A campaign whose config carries `test_contacts` is a *test campaign*: `seed_pass` takes its targets from the config, ignores `scope`, and reaches the sender without census or Serper. Its addresses stay in the config — never in the shared `jurisdictions.contact_email` — and its sends skip the shared cooldown stamp (`stamp_cooldown=False`), so rehearsing against a real county neither redirects nor throttles the campaigns that mail it for real. Consumers only react: `scraper.py` (search → crawl → pick a contact), `sender.py` (initial requests + follow-up/clarification/fee-agreement jobs), `receiver.py` (match → classify → log → react), with `attachments.py`, `fees.py`, `digest.py` shared between them.
- The rest mirrors `core/`'s layout one-for-one: `constants.py` (every hardcoded number), `config.py` (the §11 per-campaign YAML, pydantic — the whole document is stored in `campaigns.config_yaml` and re-validated on *every* read of the row, so `extra="forbid"` means retiring a config key breaks documents already in the database), `domain.py` (rows + the §4 thread state machine `check_transition`), `messages.py`, `errors.py`, `fakes/`, `adapters/`, `_protocol_checks.py`.
